...
def ecran_accueil(fenetre, images):
    ...
    # Fond : background.jpg deja charge, sinon degarde sombre
    fond_accueil = pygame.transform.scale(images["fond"], (LARGEUR, HAUTEUR))

    # Overlay sombre par-dessus le fond
    ...
    # Boutons : JOUER et QUITTER uniquement
    btn_w, btn_h = 280, 55
    btns = [
        {"label": "JOUER",   "action": "jouer",   "y": HAUTEUR - 230},
        {"label": "QUITTER", "action": "quitter",  "y": HAUTEUR - 160},
    ]
    for b in btns:
    ...
    while True:
        ...
        # Brume en bas
        brume = pygame.Surface((LARGEUR, 120), pygame.SRCALPHA)
        for bx in range(0, LARGEUR, 4):
            bh = int(40 + 30 * math.sin(bx * 0.02 + tick * 0.03))
            ba = int(60 + 30 * math.sin(bx * 0.015 + tick * 0.02))
            pygame.draw.line(brume, (180, 200, 255, ba), (bx, 120), (bx, 120 - bh))
        fenetre.blit(brume, (0, HAUTEUR - 120))
        ...
        if highscores:
            ...
            fenetre.blit(hs_titre, hs_titre.get_rect(center=(LARGEUR - 140, 320)))
            for i, hs in enumerate(highscores[:3]):
                c   = [(255, 215, 0), (192, 192, 192), (205, 127, 50)][i]
                txt = font_hs.render(f"{i+1}. {hs['score']}  ({hs['date']})", True, c)
                fenetre.blit(txt, txt.get_rect(center=(LARGEUR - 140, 345 + i * 25)))

        # Boutons JOUER et QUITTER
        mx, my = pygame.mouse.get_pos()
        for b in btns:
            survol = b["rect"].collidepoint(mx, my)
            couleur_fond = (80, 60, 20, 220) if survol else (30, 20, 5, 180)
            couleur_bord = (255, lueur, 0)   if survol else (140, 100, 20)
            bsurf = pygame.Surface((btn_w, btn_h), pygame.SRCALPHA)
            pygame.draw.rect(bsurf, couleur_fond, bsurf.get_rect(), border_radius=10)
            pygame.draw.rect(bsurf, (*couleur_bord, 255), bsurf.get_rect(), width=2, border_radius=10)
            fenetre.blit(bsurf, b["rect"].topleft)
            couleur_txt = (255, lueur, 50) if survol else (220, 200, 140)
            txt = font_bouton.render(b["label"], True, couleur_txt)
            fenetre.blit(txt, txt.get_rect(center=b["rect"].center))

        # Indication clavier
        hint = font_hs.render("ESPACE = Jouer  |  ECHAP = Quitter", True, (120, 120, 120))
