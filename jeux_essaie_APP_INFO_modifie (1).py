...
def generer_son_collision():
    """Génère un son de collision synthétique (bip court descendant)."""
    freq_echantillon = 44100
    duree = 0.15           # secondes
    nb_echantillons = int(freq_echantillon * duree)
    t = np.linspace(0, duree, nb_echantillons, endpoint=False)
    # Fréquence qui descend rapidement (effet d'impact)
    freq = 440 * np.exp(-8 * t)
    onde = (np.sin(2 * np.pi * freq * t) * 32767 * np.exp(-10 * t)).astype(np.int16)
...
def generer_son_game_over():
    """Génère un son de game over synthétique (notes graves descendantes)."""
    freq_echantillon = 44100
    duree_totale = 1.5
    nb_total = int(freq_echantillon * duree_totale)
    onde_totale = np.zeros(nb_total, dtype=np.float64)

    # Trois notes graves qui descendent
    notes = [(300, 0.0, 0.5), (200, 0.4, 0.5), (120, 0.9, 0.6)]
    for freq_note, debut, duree_note in notes:
        n = int(freq_echantillon * duree_note)
        t = np.linspace(0, duree_note, n, endpoint=False)
        seg = np.sin(2 * np.pi * freq_note * t) * np.exp(-3 * t)
        idx = int(debut * freq_echantillon)
        fin = min(idx + n, nb_total)
        onde_totale[idx:fin] += seg[:fin - idx]

    # Normaliser
    maxi = np.max(np.abs(onde_totale))
    if maxi > 0:
        onde_totale = onde_totale / maxi
    onde_int = (onde_totale * 28000).astype(np.int16)
...
def lire_images():
    ...
    # ── Sons ──────────────────────────────────────────────────────────────────
    # Son de collision (fichier OGG ou synthétique si absent)
    try:
        son_collision = pygame.mixer.Sound("collision.OGG")
        son_collision.set_volume(0.6)
        images["son_collision"] = son_collision
        print("Son collision chargé depuis collision.OGG")
    except:
        print("collision.OGG non trouvé → son synthétique généré")
        images["son_collision"] = generer_son_collision()

    # Son game over (fichier OGG ou synthétique si absent)
    try:
        son_game_over = pygame.mixer.Sound("gameover.OGG")
        son_game_over.set_volume(0.8)
        images["son_game_over"] = son_game_over
        print("Son game over chargé depuis gameover.OGG")
    except:
        print("gameover.OGG non trouvé → son synthétique généré")
        images["son_game_over"] = generer_son_game_over()
    ...
    # ── JEU EN COURS ─────────────────────────────────────────────────────────
        ...
        for fruit in fruits:
            ...
            if perso.collide(fruit):
                vies_avant = perso.vies
                perso.appliquer_demi_degats()
                if perso.vies < vies_avant and images["son_collision"]:
