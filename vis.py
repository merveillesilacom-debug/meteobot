...
while continuer:
    ...
    if game_over:
        ...
        if perso.score >= score_prochaine_vague:
            ...
            score_prochaine_vague += 1000
            if vague_actuelle == 2:
                fond_v2_img = pygame.transform.scale(images["fond_v2"], (LARGEUR, HAUTEUR * 2))
                fond_v2_y   = HAUTEUR
            afficher_annonce_vague(fenetre, vague_actuelle)
            timers["balle"] = pygame.time.get_ticks()

        if vague_actuelle >= 2 and fond_v2_y > -HAUTEUR:
            fond_v2_y -= fond.vitesse
            fenetre.blit(fond_v2_img, (0, fond_v2_y))
            if fond_v2_y <= -HAUTEUR:
