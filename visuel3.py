...
# ── Classes (ElementGraphique et héritiers) ───────────────────────────────────
...
def lire_images():
    ...
    images["fond_v2"]   = pygame.image.load("landscape.png").convert()
...
def gerer_objets(liste, perso, on_collision):
    restants = []
    for obj in liste:
        obj.deplacer()
        if obj.hors_ecran():
...
def ecran_accueil(fenetre, images):
    ...
    for y in range(HAUTEUR):
        ...
        pygame.draw.line(fond_accueil,
                         (int(10 + ratio * 30), int(10 + ratio * 20), int(40 + ratio * 80)),
                         (0, y), (LARGEUR, y))
    ...
    rect_btn  = pygame.Rect(LARGEUR // 2 - 130, HAUTEUR - 140, 260, 65)
...
while continuer:
    ...
    if game_over:
        ...
        if perso.score >= score_prochaine_vague:
            ...
            score_prochaine_vague += 1000
            if vague_actuelle == 2:
                fond.image     = pygame.transform.scale(images["fond_v2"], (LARGEUR, HAUTEUR))
                transition_alpha = 255
            afficher_annonce_vague(fenetre, vague_actuelle)
            timers["balle"] = pygame.time.get_ticks()

        if transition_alpha > 0:
            surf_transition = pygame.Surface((LARGEUR, HAUTEUR))
            surf_transition.blit(fond.image, (0, 0))
            surf_transition.set_alpha(transition_alpha)
            fenetre.blit(surf_transition, (0, 0))
            transition_alpha = max(0, transition_alpha - 3)
        ...
        if now - timers["balle"] >= 6000:
            max_balles = min(3 + vague_actuelle, 5)
        ...
        fruits               = gerer_objets(fruits,               perso, on_fruit)
        bonus_immunite_liste = gerer_objets(bonus_immunite_liste,  perso, on_immunite)
        bonus_ralenti_liste  = gerer_objets(bonus_ralenti_liste,   perso, on_ralenti)
        bonus_coeur_liste    = gerer_objets(bonus_coeur_liste,     perso, on_coeur)
        ...
        if perso.immunite_active:
            s = font_hud.render(f"IMMUNITE: {(perso.duree_immunite - perso.comp_immunite) // 30}s",
                                True, (255, 215, 0))
            fenetre.blit(s, s.get_rect(center=(LARGEUR // 2, 20)))

        if perso.ralenti_actif:
            s = font_hud.render(f"SLOW: {(perso.duree_ralenti - perso.comp_ralenti) // 30}s",
                                True, (50, 200, 255))
            fenetre.blit(s, s.get_rect(center=(LARGEUR // 2, 55)))

        fenetre.blit(font_leg.render(
            "Fruit = -1/2 vie  |  OR = Immunite  |  BLEU = Ralentit  |  ROSE = +1/2 vie",
