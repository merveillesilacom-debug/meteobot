...
# Fonds par vague : vague 1 = sky, vague 2 = ville, vague 3 = mars
FONDS_VAGUE = {
    1: "sky1.png",
    2: "background.png",
    3: "marsmid.png",
...
AMBIANCES_VAGUE = {
    1: {"nom": "Dans les nuages",    "couleur_titre": (100, 200, 255)},
    2: {"nom": "Ville nocturne",     "couleur_titre": (100, 150, 255)},
    3: {"nom": "Surface de Mars",    "couleur_titre": (255, 100, 60) },
}
...
def lire_images(fenetre):
    ...
    # Fonds par vague (pré-scalés)
    for vague, nom_fichier in FONDS_VAGUE.items():
        try:
            img = pygame.image.load(nom_fichier).convert()
            images[f"fond_{vague}"] = pygame.transform.scale(img, (largeur, hauteur))
            print(f"Fond vague {vague} ({nom_fichier}) chargé.")
        except Exception as e:
            print(f"Fond {nom_fichier} non trouvé : {e}")
            # Fond de remplacement coloré
            surf = pygame.Surface((largeur, hauteur))
            couleurs = {1: (80, 160, 220), 2: (20, 20, 80), 3: (120, 40, 20)}
            surf.fill(couleurs.get(vague, (30, 30, 30)))
            images[f"fond_{vague}"] = surf
...
class Fond:
    """Fond défilant verticalement, change d'image selon la vague."""
    def __init__(self, fenetre, vitesse=2):
        self.fenetre  = fenetre
        ...
        self.haut     = lh
        self.actif    = False
        self.image    = None  # sera défini par changer_vague()

    def changer_vague(self, images, vague):
        """Change l'image de fond selon la vague."""
        cle = f"fond_{vague}"
        # Fallback sur la dernière vague disponible
        while cle not in images and vague > 1:
            vague -= 1
            cle = f"fond_{vague}"
        self.image = images.get(cle)

    def deplacer(self):
        if not self.actif or self.image is None:
            return
...
class Perso(ElementGraphique):
    ...
    def afficher(self):
        if self.inv and (self.comp_inv // 5) % 2 == 0:
            ...
            # Halo doré pulsant
            pulse = int(8 + 4 * math.sin(pygame.time.get_ticks() * 0.01))
            pygame.draw.circle(
                self.fenetre, (255, 215, 0),
                self.rect.center,
                max(self.rect.w, self.rect.h) // 2 + pulse,
                3
            )
            self.fenetre.blit(self.image, self.rect)
        else:
...
class BonusBase(ElementGraphique):
    """Classe de base pour tous les bonus tombants."""
    def __init__(self, fenetre, couleur_fond, couleur_bord, texte, taille_texte=32):
        lw     = fenetre.get_width()
        taille = 48
        surf   = pygame.Surface((taille, taille), pygame.SRCALPHA)
        pygame.draw.circle(surf, couleur_fond, (taille//2, taille//2), taille//2)
        ...
        super().__init__(surf, fenetre, x, -taille)
...
class BonusImmunite(BonusBase):
    def __init__(self, fenetre):
        super().__init__(fenetre, (255, 215, 0), (200, 160, 0), "+1", 32)

class BonusRalenti(BonusBase):
    def __init__(self, fenetre):
        super().__init__(fenetre, (50, 150, 255), (30, 100, 200), "SLOW", 26)

class BonusCœur(BonusBase):
    def __init__(self, fenetre):
        super().__init__(fenetre, (255, 80, 130), (200, 40, 90), "+½", 32)
...
def afficher_annonce_vague(fenetre, largeur, hauteur, vague):
    ambiance = AMBIANCES_VAGUE.get(vague, {"nom": "Survivez encore...", "couleur_titre": (255, 255, 255)})
    couleur  = ambiance["couleur_titre"]
    nom      = ambiance["nom"]
    ...
    font_grand = pygame.font.Font(None, 90)
    font_nom   = pygame.font.Font(None, 46)

    pygame.draw.line(fenetre, couleur, (80, hauteur//2 - 65), (largeur - 80, hauteur//2 - 65), 2)

    surf_v = font_grand.render(f"VAGUE  {vague}", True, couleur)
    fenetre.blit(surf_v, surf_v.get_rect(center=(largeur//2, hauteur//2 - 10)))

    surf_n = font_nom.render(nom, True, (220, 220, 220))
    fenetre.blit(surf_n, surf_n.get_rect(center=(largeur//2, hauteur//2 + 60)))

    pygame.draw.line(fenetre, couleur, (80, hauteur//2 + 90), (largeur - 80, hauteur//2 + 90), 2)
...
# Chargement du fond de la vague 1
fond.changer_vague(images, vague_actuelle)
afficher_annonce_vague(fenetre, largeur, hauteur, vague_actuelle)

# Réinitialiser les timers après l'accueil
...
while continuer:
    ...
    if en_pause and not game_over:
        ...
        txt_hint  = font_hint.render("Appuyez sur P pour reprendre", True, (200, 200, 200))
    ...
    if game_over:
        ...
        # Overlay sombre
        overlay_go = pygame.Surface((largeur, hauteur), pygame.SRCALPHA)
        overlay_go.fill((0, 0, 0, 120))
        fenetre.blit(overlay_go, (0, 0))

        font_go = pygame.font.Font(None, 110)
        surf    = font_go.render("GAME OVER", True, (255, 40, 40))
        fenetre.blit(surf, surf.get_rect(center=(largeur // 2, hauteur // 2 - 70)))

        font_sc = pygame.font.Font(None, 60)
        surf    = font_sc.render(f"Score : {perso.score}", True, (255, 255, 255))
        fenetre.blit(surf, surf.get_rect(center=(largeur // 2, hauteur // 2 + 10)))

        temps_survie = perso.score // 60  # 60 FPS maintenant
        font_t = pygame.font.Font(None, 38)
        surf   = font_t.render(f"Temps de survie : {temps_survie} s", True, (200, 200, 200))
        ...
        fenetre.blit(surf, surf.get_rect(center=(largeur // 2, hauteur // 2 + 115)))
        for i, hs in enumerate(highscores[:3]):
            couleurs_hs = [(255, 215, 0), (192, 192, 192), (205, 127, 50)]
            txt = font_hs.render(f"{i+1}. {hs['score']}  ({hs['date']})", True, couleurs_hs[i])
            fenetre.blit(txt, txt.get_rect(center=(largeur // 2, hauteur // 2 + 148 + i * 28)))

        font_r = pygame.font.Font(None, 30)
        surf   = font_r.render("ESPACE pour rejouer  |  ECHAP pour quitter", True, (200, 200, 200))
        fenetre.blit(surf, surf.get_rect(center=(largeur // 2, hauteur - 40)))

    # ── JEU EN COURS ──────────────────────────────────────────────────────────
    else:
        perso.afficher()
        perso.deplacer(largeur, hauteur)
        perso.score += 1

        temps_actuel = pygame.time.get_ticks()

        # ── Changement de vague + fond ──
        ...
        # Bonus immunité — déplacement + collision
        bonus_restants = []
        ...
        if perso.immunite_active:
            secondes = (perso.duree_immunite - perso.comp_immunite) // 60
        ...
        if perso.ralenti_actif:
            secondes = (perso.duree_ralenti - perso.comp_ralenti) // 60
        ...
        fenetre.blit(font_leg.render(
            "Immunite = or  |  SLOW = bleu  |  Coeur = rose  |  P = Pause",
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_p and not game_over and not en_pause:
                en_pause = True
