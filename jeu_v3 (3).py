...
NIVEAUX = {
    1: {"fichier": "background.png", "nom": "Verdure",        "couleur": (80,  200, 80) },
    2: {"fichier": "sky1.png",       "nom": "Dans les cieux", "couleur": (100, 180, 255)},
    3: {"fichier": "fond_nuit.png",  "nom": "Ville nocturne", "couleur": (80,  100, 255)},
    4: {"fichier": "marsmid.png",    "nom": "Surface de Mars","couleur": (255, 100, 60) },
}
...
def lire_images(fenetre):
    ...
    for num, niveau in NIVEAUX.items():
        try:
            ...
            images[f"fond_{num}"] = pygame.transform.scale(img, (largeur, hauteur))
    ...
    try:
        ...
        print("bonus.wav non trouve (optionnel)")
...
class Fond:
    """Fond défilant, change d'image selon le niveau."""
    ...
    def changer_niveau(self, images, niveau):
        cle = f"fond_{niveau}"
        while cle not in images and niveau > 1:
            niveau -= 1
            cle = f"fond_{niveau}"
    ...
    def afficher(self):
        if self.inv and (self.comp_inv // 5) % 2 == 0:
            ...
            pygame.draw.circle(
                ...
                self.rect.center,
                max(self.rect.w, self.rect.h) // 2 + 8, 3
            )
            self.fenetre.blit(self.image, self.rect)
        else:
            self.fenetre.blit(self.image, self.rect)
...
class BonusCœur(ElementGraphique):
    def __init__(self, fenetre):
        ...
        txt  = font.render("+1/2", True, (255, 255, 255))
        surf.blit(txt, txt.get_rect(center=(taille//2, taille//2)))
        x = random.randint(0, lw - taille)
        super().__init__(surf, fenetre, x, -taille)
        self.vitesse = random.uniform(1.5, 3.0)
...
def dessiner_barre_vie(fenetre, vies, max_vies, x, y, largeur_barre=200, hauteur_barre=22):
    ...
    vies_ent     = int(vies)
    demi_affiche = ".5" if (vies - vies_ent) >= 0.5 else ""
...
def afficher_annonce_niveau(fenetre, largeur, hauteur, numero):
    niveau  = NIVEAUX.get(numero, {"nom": "Survivez...", "couleur": (255, 255, 255)})
    ...
    surf_v = font_grand.render(f"NIVEAU  {numero}", True, couleur)
...
while continuer:
    ...
    # ── JEU EN COURS ──────────────────────────────────────────────────────────
        ...
        if perso.score >= score_prochain_niveau and niveau_actuel < 4:
            niveau_actuel         += 1
            score_prochain_niveau += 1000
            fond.changer_niveau(images, niveau_actuel)
            afficher_annonce_niveau(fenetre, largeur, hauteur, niveau_actuel)
        ...
        # Nouvelle balle
        max_balles_niveau = min(3 + niveau_actuel, nbre_max_balle)
        if temps_actuel - td_balle >= intervalle_balle:
            if len(balles) < max_balles_niveau:
        ...
        fenetre.blit(font_hud.render(f"Niveau: {niveau_actuel}", True, (255, 220, 50)), (10, 45))

        pct_diff = int((facteur_vitesse - 1.0) / 2.0 * 100)
        fenetre.blit(font_leg.render(f"Difficulte: {pct_diff}%", True, (200, 200, 255)), (10, 80))
        ...
        fenetre.blit(font_leg.render(
            "Or = Immunite  |  Bleu = Ralentit balles  |  Rose = +vie  |  P = Pause",
