...
def lire_images():
    ...
    try:
        images["fond_v3"] = pygame.image.load("sky.png").convert()
    except:
        surf = pygame.Surface((LARGEUR, HAUTEUR))
        for y in range(HAUTEUR):
            t = y / HAUTEUR
            pygame.draw.line(surf, (int(135 + t*30), int(180 + t*40), int(235 + t*20)), (0, y), (LARGEUR, y))
        images["fond_v3"] = surf
...
while continuer:
    ...
    if game_over:
        ...
        if perso.score >= score_prochaine_vague:
            vague_actuelle        += 1
            score_prochaine_vague += 1000
            cle = {2: "fond_v2", 3: "fond_v3"}.get(vague_actuelle)
            if cle:
                nouveau_fond_img = pygame.transform.scale(images[cle], (LARGEUR, HAUTEUR))
                nouveau_fond_y   = -HAUTEUR
            afficher_annonce_vague(fenetre, vague_actuelle)
